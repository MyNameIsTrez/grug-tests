...
static struct l l_definition;

void define_l(char *group, char *name) {
	l_definition = (struct l){
		.group = group,
		.name = name,
	};
}
...
int main(int argc, char *argv[]) {
	...
	assert(strcmp(get(handle, "define_type"), "l") == 0);

	#pragma GCC diagnostic push
	#pragma GCC diagnostic ignored "-Wpedantic"
	grug_define_fn_t define = get(handle, "define");
	define();
	assert(strcmp(l_definition.group, "military") == 0);
	assert(strcmp(l_definition.name, "foo") == 0);
